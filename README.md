...
Default/Preferences is the profile: browser settings, enabled extensions, active theme.  
system/etc/opt/chrome holds the machine policy that disables the default browser check; it is installed to /etc/opt/chrome.  
system/opt/google/chrome holds the external extension files that make Chrome auto-install the chosen extensions; they are installed to /opt/google/chrome.  
hooks/pre-commit is the git hook that keeps Default/Preferences pretty-printed (see below).

The profile is applied on the target machine by the Pyntara provisioning tool. Files under system/ map to the same absolute locations in the filesystem root.
